...
void ifpoison(Monster *Ori , int mc , point base[21][80])
{
  for(int i = 0 ; i < mc ; i ++)
    { 
      if(base[(Ori+i)->x][(Ori+i)->y].color == GREEN)
	(Ori+i)->HP -=10;
...
bool killed(WINDOW **game,Monster*Ori , int mc , int px ,int py , Player *p , char *s)
{
  ...
  for(int i = 0 ; i < mc ; i++)
      ...
      if((Ori+i)->x == px && (Ori+i)->y == py && (Ori+i)->Dead == false)
	{
	  if((Ori+i)->HP <= n)
	    {
	      if((Ori+i)->isBoss || ((Ori+i)->isBoss && (Ori+i)->Dead == true))
		{
...
char PCMove( Charactor *M ,  point base[21][80], WINDOW **game , char store,  Monster *Ori,int mc,int stair, int Dstair , vector<objects> *ob)
{
   ...
   while(check == true)
  {
	...
	case 32:
	  check =false;
	  killed(game , Ori , mc , (M->x)+1, (M->y)-1 , (Player*)M,&store);
	  store =  base[M->x][M->y].value;
	  break;
	case 'Q':
	  check = false;
