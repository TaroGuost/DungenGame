	   ...
	   if(!check)
		...
		{		  
		  if(base[i][j].ob != '.')
			...
			{
			  init_pair(base[i][j].color,base[i][j].color,COLOR_BLACK);
			  wattron(*game, COLOR_PAIR(base[i][j].color));
			  mvwprintw(*game , i , j , "%c" , base[i][j].fake);
			  wattroff(*game, COLOR_PAIR(base[i][j].color));
			}
		    }
		}
	    }
...
void PrintList(WINDOW **list,int mc, int px ,int py, Monster *M,int n)
{
  ...
  mvwprintw(*list,0 , 10 , "%s" , "MonsterList");
...
void printspell(WINDOW **list , Player *p , int highlight)
{
  mvwprintw(*list , 0 , 10 , "SPELL LIST");
  
  for(int i = 0 ; i < p->spell.size(); i ++)
    {
      if(i == highlight)
	{
	  wattron(*list , A_REVERSE);
	  mvwprintw(*list , i+1 , 1,"Type:%s Name: %s" ,p->spell.at(i).type.c_str(), p->spell.at(i).name.c_str());
	  wattroff(*list , A_REVERSE);
	}
      else
	{
